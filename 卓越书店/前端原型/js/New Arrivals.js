...
function getDat(page_num){
	...
	$.ajax({
		url:"http://192.168.1.138:8080/api/product/list",
		type:"get",
		dataType:"json",
		cache:false,
		xhrFields:{
	...
	// }
	 for (var i = 0; i < 8; i++) {
        ...
        for (var x = 0; x <product_image.length; x++) {
            ...
            "<li ><img src='"+ "http://192.168.1.138:8080" + product_image[x].picture + "' class='im' onclick='showActivityBo(this)'> </li>" +
            "</div>";
...
function showActivityBo(item) {
    window.location.href = "Look at.html?id=" + $(item).closest("div").data("id");
    return;
}